            ...
            \begin{indt}{\subsubsection{Exemple}}
                \begin{center}
                    \begin{tikzpicture}[scale=1.5]
                        ...
                        \draw[->] (0) to node [above right] {$-2$} (2);
                        \draw[->] (2) to node [below right] {$2$} (3);
                        \draw[->] (3) to node [below left] {$-1$} (1);
                        \draw[->] (1) to node [above left] {$4$} (0);
            ...
            \begin{indt}{\subsubsection{Exemple}}
                \begin{center}
                    \begin{tikzpicture}[scale=1.5]
                        ...
                        \draw[->, color=ff4500] (0) to node [below left] {$5$} (1);
                        \draw[->, color=ff4500] (0) to node [above left] {$3$} (2);
                        \draw[->] (1) to node [below] {$6$} (3);
                        \draw[->] (1) to [out=80, in=-80] node [right] {$1$} (2);
                        \draw[->] (2) to [out=-100, in=100] node [left] {$2$} (1);
                        \draw[->] (3) to [out=80, in=-80] node [right] {$7$} (4);
                        \draw[->, color=ff4500] (4) to [out=-100, in=100] node [left] {$1$} (3);
                        \draw[->] (1) to node [below right] {$4$} (4);
                        \draw[->, color=ff4500] (2) to node [above] {$6$} (4);
                        \draw[->] (3) to [looseness=1.3, out=-135, in=-100] node [below] {$3$} (0);
                    \end{tikzpicture}
                \end{center}
...
int* dijkstra(graphe g, int n, int s) {
    ...
    free(w);
    //free memory for f if necessary here
    return p;
}\end{lstlisting}
                ...
                \begin{indt}{$\bullet$ Temporelle : cela dépend de l'implémentation du type \texttt{fp} :}
                    ...
                    \begin{indt}{$-$ Implémentation à l'aide d'un tas :}
                        ...
                        Cela peut être meilleurs s'il y a peu d'arêtes, par exemple si $\abs A = \mathcal O(\abs S)$, ce qui donne $\mathcal O(\abs S \log \abs S)$

                        (H.P) : Avec une implémentation à l'aide de \textit{tas de Fibonacci}, on obtient $\mathcal O\!\lr{\abs S \log \abs S + \abs A}$.
                    \end{indt}
                \end{indt}
            \end{indt}
        \end{indt}
